import { getBestResults } from "./firbaseDb.js";
...
let values = await getBestResults();
  ...
  const firstHalfValues = values.length % 2 ? values.slice(0, half + 1) : values.slice(0, half);
...
const step = actualWidth / (data[data.length - 1] - data[0]);
let begin_x = 45 + (Q1 - data[0]) * step;
let end_x = 45 + (Q3 - data[0]) * step;
let median_x = 45 + (Q2 - data[0]) * step;
...
ctx.fillStyle = "blue";
ctx.font = "14px Arial";
ctx.fillText(data[0], 40, W_height-20);
...
ctx.fillText(data[data.length-1], W_width - 50, W_height-20);
...
ctx.fillText(Q2, median_x-10, W_height-20);
